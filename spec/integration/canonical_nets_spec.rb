...
RSpec.describe 'canonical nets' do
  ...
  it 'prefills suggested merge name with the most common underlying name' do
    admin = create_user(call_sign: 'K1ADMIN')
    admin.update!(admin: true)
    first = Tables::CanonicalNet.create!(canonical_name: 'Metro Weather Net')
    second = Tables::CanonicalNet.create!(canonical_name: 'Metro WX Net')
    create_closed_net(canonical_net: first, name: 'Metro WX', started_at: 4.days.ago)
    create_closed_net(canonical_net: first, name: 'Metro WX', started_at: 3.days.ago)
    create_closed_net(canonical_net: second, name: 'Metro WX', started_at: 2.days.ago)
    create_closed_net(canonical_net: second, name: 'Metro Weather Net', started_at: 1.day.ago)
    Tables::SuggestedCanonicalNetMerge.create!(
      signature: '146.52::2m::metro-common',
      frequency: '146.52',
      band: '2m',
      normalized_name: 'metro weather net',
      canonical_net_ids: [first.id, second.id].to_json,
    ...
    expect(last_response.status).to eq(200)
    expect(last_response.body).to include('name="canonical_name"')
    expect(last_response.body).to include('value="Metro WX"')
  end
